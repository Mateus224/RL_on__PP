...
Transition_dtype = np.dtype([('timestep', np.int32), ('pcl', np.float32, (3,1620)),('pose', np.float32, (7)), ('action', np.int32), ('reward', np.float32), ('nonterminal', np.bool_)])
blank_trans = (0, np.zeros((3,1620), dtype=np.float32),np.zeros((7), dtype=np.float32), 0, 0.0, False)
...
class ReplayMemory():
  ...
  def append(self, state, action, reward, terminal):
    ...
    #pcl= np.swapaxes(pcl,0,2)
    state_pcl=torch.Tensor(pcl)    

    #pose= np.swapaxes(pose,0,2)
    state_pose=torch.Tensor(pose)  
  ...
  def _get_samples_from_segments(self, batch_size, p_total):
    ...
    pcl_state = np.copy(transitions['pcl'])
    pose_state = np.copy(transitions['pose'])
    pcl_state = np.squeeze(pcl_state)
    pose_state = np.squeeze(pose_state)
    
    pcl_state_ = torch.tensor(pcl_state[:,0], device=self.device, dtype=torch.float32)
    states_pose_ = torch.tensor(pose_state[:,0], device=self.device, dtype=torch.float32)
    pcl_next_state_ = torch.tensor(pcl_state[:,1], device=self.device, dtype=torch.float32)
    pose_next_state_ = torch.tensor(pose_state[:,1], device=self.device, dtype=torch.float32)
    ...
    return probs, idxs, tree_idxs, pcl_state_, states_pose_, actions, R, pcl_next_state_, pose_next_state_, nonterminals

  def sample(self, batch_size):
    p_total = self.transitions.total()  # Retrieve sum of all priorities (used to create a normalised probability distribution)
    probs, idxs, tree_idxs, states_pcl, states_pose, actions, returns, next_pcl_states, next_pose_states, nonterminals = self._get_samples_from_segments(batch_size, p_total)  # Get batch of valid samples
    probs = probs / p_total  # Calculate normalised probabilities
    capacity = self.capacity if self.transitions.full else self.transitions.index
    weights = (capacity * probs) ** -self.priority_weight  # Compute importance-sampling weights w
    weights = torch.tensor(weights / weights.max(), dtype=torch.float32, device=self.device)  # Normalise by max importance-sampling weight from batch
    return tree_idxs, states_pcl, states_pose, actions, returns, next_pcl_states, next_pose_states, nonterminals, weights
  ...
  def __next__(self):
    ...
    transitions[blank_mask] = blank_trans
    pcl_state = torch.tensor(transitions['pcl'], dtype=torch.float32, device=self.device).div_(255)  # Agent will turn into batch
    pose_state = torch.tensor(transitions['pose'], dtype=torch.float32, device=self.device).div_(255)  # Agent will turn into batch
    self.current_idx += 1
    return pcl_state, pose_state
