...
void DroneSimpleController::Load(physics::ModelPtr _model, sdf::ElementPtr _sdf)
{
  ...
  /*
  ignition::math::Vector3d orig_pos;
  ignition::math::Quaterniond orig_at(1,0,0,0);
  orig_pos.Set(0,0,1);
  orig_at.Set(1,0,0,0);
  ignition::math::Pose3d orig_pose(orig_pos, orig_at);
  link->SetRelativePose(orig_pose);
   */
...
void DroneSimpleController::SwitchModeCallback(const std_msgs::BoolConstPtr& msg){
    m_velMode = msg->data;
    ROS_INFO("%s","\nQuadrotor received new command!!");
}
...
void DroneSimpleController::UpdateDynamics(double dt){
  ...
  //  if (imu_subscriber_.getTopic()=="")
   ...
   // if (state_topic_.empty())
    ...
    //if(pose.Pos().Y()<0.08 && pose.Pos().Y()>0.00){
    //if (enter==0){
    //  begin =ros::Time::now();
    //  enter=1;
    //  ROS_INFO("%s", "entering");
    //}}
    //force.Set(0.0, 1000.0, 0.0);
    //std::cout<<pose.Pos().Y()<<"\n";
    //if (pose.Pos().Y()>1 && pose.Pos().Y()<1.1){
      
    /*  ros::Time end =ros::Time::now();
      ros::Duration time=end-begin;
      //std::end<<begin<<"\n";
      link->ResetPhysicsStates();
      //ignition::math::Vector3d orig_pos;
      //ignition::math::Quaterniond orig_at(0,0,0,0);
      //orig_pos.Set(0,0,1);
      //orig_at.Set(0,0,0,1);
      //ignition::math::Pose3d orig_pose(orig_pos, orig_at);
      //link->SetRelativePose(orig_pose);
      if (enter==1){
        ROS_INFO("%s", "gettingOut");
        ROS_INFO("%f" ,time.toSec());
        enter=2;
    }
    ...
    if (navi_state == FLYING_MODEL) {
      //std::cout<<pose.Pos().Y()<<"2\n";
      //link->AddForce(force );
      //link->AddTorque(torque * 1.5); 
